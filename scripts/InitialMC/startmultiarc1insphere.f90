PROGRAM MULTIARC1SPHEREINITIALTXT
    IMPLICIT NONE
    INTEGER,PARAMETER:: nring=6, npart= 200, nsmall=50, mcniter = 10000, nloop = 1
    REAL*8,PARAMETER:: lb = 1.0, pi = 3.1417, radsphere= 7.250d0, sigma=0.80d0, T = 1.0d0, eps = 1.0d0, sprconst= 200.0d0
    REAL*8,PARAMETER:: radius= 3.0, theta = (2*pi)/npart, dela = lb/10, rs = (1.122 + 2.0d0)
...
!-----------------------------------------------------------------------------------------------
    ...
    !OPEN(unit=42,file='arc1_200_spherestart_n6_vf0.025.dat',status='unknown')
    OPEN(unit=40,file='arc1_200_spherestart_n6_vf0.2.txt',status='unknown')
    WRITE(40,*) "LAMMPS data file for ring polymer of length 200"
    WRITE(40,*) ""
    WRITE(40,*) nring*npart, "atoms"
    WRITE(40,*) nring*(npart+1),"bonds"
    WRITE(40,*) ""
    WRITE(40,*) nring*(nloop+1), "atom types"
    WRITE(40,*) 1, "bond types"
    WRITE(40,*) ""
    WRITE(40,*) "Masses"
    WRITE(40,*) ""
    DO i = 1, nring*(nloop+1)
        WRITE(40,*) i, 1.0
    ENDDO
    WRITE(40,*) ""
    WRITE(40,*) "Atoms"
...
!------------------------------------------------------------------------------------------------------------------
    ...
    DO time= 1, mcniter  
        ...
        DO mm= 1,(npart*nring)
          ...
          if ((b.gt.0).and.(b.le.2)) then
            if ((c.eq.0).and.(b.ne.2)) then
        ...
        !if (time.gt.10000000) then
            !if (mod(time,10000)==0) then
                !Do i = 1, nring
                    !DO j = 1, npart
                        !k = npart*(i-1) + j
                        !WRITE(42,*) k, pos(k,1), pos(k,2), pos(k,3)
                    !ENDDO
                !ENDDO
            !ENDIF
        !ENDIF 
    ...
    DO i= 1, nring
        ...
        !WRITE(50,*) pos(i,1), pos(i,2), pos(i,3)
            if ((j.ge.51).and.(j.le.100)) then
                WRITE(40,*) k, 0, 2*(i-1)+1, pos(k,1), pos(k,2), pos(k,3), 0, 0, 0
            else   
                WRITE(40,*) k, 0, 2*(i-1)+2, pos(k,1), pos(k,2), pos(k,3), 0, 0, 0
            endif
        ENDDO
    ENDDO
    ...
    DO i= 1, nring
            k = i
            WRITE(40,*) (npart*nring)+k, 1, (((i-1)*npart) + 50), ((i-1)*npart + 100)
    ENDDO

END PROGRAM MULTIARC1SPHEREINITIALTXT
